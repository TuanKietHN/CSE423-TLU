 ...
 */
public class BoundedBufferMonitorLinkedList {
    private final BoundedBufferQueue buffer;
    private volatile boolean running = true;

    /**
     * Constructor khởi tạo buffer với dung lượng cho trước
     */
    public BoundedBufferMonitorLinkedList(int capacity) {
        this.buffer = new BoundedBufferQueue(capacity);
    }
    ...
    public Thread createProducer() {
        return new Thread(() -> {
            ...
            while (running) {
                try {
                    int number = random.nextInt(191) + 10; // Sinh số từ 10-200
                    buffer.put(number);
